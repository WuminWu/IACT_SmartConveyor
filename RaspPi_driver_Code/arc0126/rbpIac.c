...
void rbpMotor(int nPin)
{
	...
	if(nPin == CONV_MOTOR4 && dir == BACKWARD) 
		nPin2 = CONV_MOTOR3;
...
{
	if(state == ON){
		set_PWM_ON_PCA9685( i2c_pwm , CONV_MOTOR5 , 0x0 );
		set_PWM_OFF_PCA9685( i2c_pwm , CONV_MOTOR5 , 0x0800 );
	}else
		set_PWM_OFF_PCA9685( i2c_pwm , CONV_MOTOR5 , 0x0000 );
}
